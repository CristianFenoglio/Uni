...
int main()
{
    ...
    map<string, unsigned int> m= {{"hi", 5}, {"ciao", 2}};
    //la mappa è fatta di PAIR
    pair<string, unsigned int> coppia={"hi", 5};
    //per accedere
    cout << m["ciao"] <<endl;
    //per accedere in sequenza
    for (auto co : m)
    {
        cout<< co.first<<" " << co.second << endl;
        //first mi da la chiave, second il valore
    }
    auto in =m.insert({"ciao", 7});
    // in : coppia --> chiave è un iterator, valore è un bool
    cout << (*in.first).second << " " << in.second << endl;
    return 0;
}
